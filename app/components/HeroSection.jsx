...
const FALLBACK_SLIDES = [
  { image: '/slider1.jpeg', mobileImage: null, title: 'Restaurant', type: 'BOTH' },
  { image: '/slider2.jpeg', mobileImage: null, title: 'Hotel',      type: 'BOTH' },
];
...
// pick correct image src based on viewport
function useBannerSrc(slide) {
...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    return () => { gridObserver.disconnect(); window.removeEventListener('scroll', onScroll); };
  }, []);
    ...
    <>
      <style>{`
        ...
        @media (max-width: 767px) {
          .sector-bar { top: 56px; padding-top: 10px; padding-bottom: 10px; }
          .sector-bar.condensed { padding-top: 4px !important; padding-bottom: 4px !important; }
        }
        ...
        .sector-bar.condensed .sc-img { aspect-ratio: 3/2; }
        @media (max-width: 767px) {
          .sc-img { aspect-ratio: 3/4 !important; }
          .sector-bar.condensed .sc-img { aspect-ratio: 1/1 !important; }
        }
        ...
        @media (max-width: 767px) {
          .sc-label-below { font-size: 9px !important; max-height: 18px; }
        }
        ...
        .sector-bar.sticky .sc-label-over { opacity: 1; animation: fadeIn 0.4s ease 0.1s both; }

        /* ── product grid ── */
        @media (max-width: 767px) {
          .product-grid { grid-template-columns: repeat(2, 1fr) !important; gap: 10px !important; }
          .sector-grid  { grid-template-columns: repeat(3, 1fr) !important; gap: 8px !important; }
        }
        @media (min-width: 768px) and (max-width: 1023px) {
          .product-grid { grid-template-columns: repeat(3, 1fr) !important; }
          .sector-grid  { grid-template-columns: repeat(6, 1fr) !important; }
        }
      ...
      <div className="mt-[66px] md:mt-[66px]" style={{ display: 'flex', flexDirection: 'column' }}>
        ...
        <div
          ref={sectorRef}
          className={`sector-bar ${sectorSticky ? 'sticky' : ''} ${isCondensed ? 'condensed' : ''}`}
        >
          <div className="w-full max-w-[1150px] mx-auto px-3 md:px-6">
            <div
              className="sector-grid grid gap-3"
                ...
                return (
                  ...
                  >
                    <div className={`sc-img rounded-xl overflow-hidden border transition-all duration-300 ${isActive ? 'border-[#1e88e5]' : 'border-[#dde4ef]'}`}>
                      ...
                      <div className={`absolute inset-0 transition-opacity duration-300 ${isActive ? 'opacity-60' : 'opacity-40'}`}
        ...
        <div ref={gridRef} className="w-full bg-[#f4f6fa]" style={{ minHeight: '100vh' }}>
          <div className="w-full max-w-[1150px] mx-auto px-3 md:px-6 py-5 md:py-8">

            {/* breadcrumb + view-all */}
            <div className="flex items-center justify-between mb-4 md:mb-5 min-h-[28px] gap-2 flex-wrap">
              ...
              {activeSector && (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 md:px-4 py-1.5 bg-[#071e3d] text-white text-[10px] md:text-[11px] font-bold uppercase tracking-widest rounded-lg hover:bg-[#0a4c8a] transition-colors shrink-0"
                >
                  {t.hero_viewall}
            ...
            {gridLoading ? (
              <div className="product-grid grid grid-cols-6 gap-4">
                {Array.from({ length: 12 }).map((_, i) => (
                  <div key={i} className="rounded-xl border border-[#dde4ef] bg-white overflow-hidden animate-pulse">
                    <div className="h-28 md:h-36 bg-[#eef1f6]" />
                    <div className="p-2.5 md:p-3">
                      <div className="h-3 bg-[#eef1f6] rounded w-3/4 mb-2" />
                      <div className="h-2 bg-[#eef1f6] rounded w-1/2" />
                    </div>
                  </div>
                ))}
              </div>
            ) : gridItems.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 md:py-20 border border-dashed border-[#dde4ef] rounded-2xl bg-white">
                <p className="text-[13px] font-bold text-[#9aa3af] uppercase tracking-widest">{t.hero_notfound}</p>
              </div>
            ) : (
              <div className="product-grid grid grid-cols-6 gap-4 fade-slide-up">
                {gridItems.slice(0, 12).map((item) => (
                  <InlineCard key={item.id} item={item} layer={layer} t={t} onClick={() => handleProductClick(item)} />
                ))}
              </div>
            )}
          </div>
...
function TruckIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ width:'clamp(14px,1.4vw,22px)', height:'clamp(14px,1.4vw,22px)' }}>
...
function ExpIcon() {
  return (
    <div className="rounded-full border-2 border-white/80 flex items-center justify-center" style={{ width:'clamp(14px,1.4vw,22px)', height:'clamp(14px,1.4vw,22px)' }}>
      <span className="text-white font-extrabold" style={{ fontSize:'clamp(6px,0.6vw,10px)', lineHeight:1 }}>30+</span>
    </div>
  );
}
...
function InlineCard({ item, layer, t, onClick }) {
  ...
  return (
    <div onClick={onClick} className="group cursor-pointer rounded-xl border border-[#dde4ef] bg-white overflow-hidden hover:border-[#1e88e5] hover:shadow-[0_8px_32px_rgba(30,136,229,0.1)] transition-all duration-200">
      <div className="h-28 md:h-36 bg-[#f4f6fa] flex items-center justify-center overflow-hidden relative">
        {item.image
        ...
        {item.isGroup && (
          <div className="absolute top-1.5 right-1.5 md:top-2.5 md:right-2.5 bg-[#071e3d] text-white text-[8px] md:text-[9px] font-bold tracking-widest uppercase px-1.5 md:px-2 py-0.5 rounded-full">
            {canDrill ? t.hero_series : t.hero_variant}
          </div>
        )}
      </div>
      <div className="p-2 md:p-2.5">
        <h3 className="text-[10px] md:text-[11px] font-bold uppercase tracking-wide text-[#071e3d] leading-tight group-hover:text-[#1e88e5] transition-colors line-clamp-2">{item.name}</h3>
        <p className="text-[9px] md:text-[10px] text-[#9aa3af] mt-1 flex items-center gap-1">
