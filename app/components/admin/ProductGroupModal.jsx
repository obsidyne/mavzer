...
export default function ProductGroupModal({ product, categoryId, parentId, depth = 0, onClose, onSaved }) {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const url = isEditing ? `${API}/api/products/${product.id}` : `${API}/api/products`;
      const body = isEditing
        ? { name, description, image }
        : { name, description, image, isGroup: true, depth, ...(parentId ? { parentId } : { categoryId }) };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md bg-[#111] border border-[#1f1f1f] rounded-xl p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-white font-semibold text-base">{isEditing ? "Edit Product Group" : "Add Product Group"}</h2>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <div className="flex flex-col gap-1.5">
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Kraft Paper Bags" required
