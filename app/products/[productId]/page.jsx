    ...
    <>
      {zoomed && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center" onClick={() => setZoomed(false)}>
          <button onClick={(e) => { e.stopPropagation(); prev(); }} className="absolute left-6 top-1/2 -translate-y-1/2 text-white/60 hover:text-white text-3xl font-thin w-12 h-12 flex items-center justify-center">‹</button>
          <img src={images[active]} alt={alt} className="max-h-[85vh] max-w-[85vw] object-contain" onClick={(e) => e.stopPropagation()} />
          <button onClick={(e) => { e.stopPropagation(); next(); }} className="absolute right-6 top-1/2 -translate-y-1/2 text-white/60 hover:text-white text-3xl font-thin w-12 h-12 flex items-center justify-center">›</button>
          <button onClick={() => setZoomed(false)} className="absolute top-5 right-6 text-white/60 hover:text-white text-xl">✕</button>
          {images.length > 1 && (
            <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex gap-1.5">
              {images.map((_, i) => (
                <button key={i} onClick={(e) => { e.stopPropagation(); setActive(i); }} className={`w-1.5 h-1.5 rounded-full transition-colors ${i === active ? "bg-white" : "bg-white/30"}`} />
              ))}
            </div>
          )}
        </div>
      ...
      <div className="w-full md:w-72 md:shrink-0 border-b md:border-b-0 md:border-r border-[#dde4ef] flex flex-col" style={{ minHeight: 200 }}>
        <div className="flex-1 bg-[#f8fafc] relative overflow-hidden cursor-zoom-in group" style={{ minHeight: 220 }} onClick={() => setZoomed(true)}>
          <img src={images[active]} alt={alt} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" style={{ maxHeight: 340 }} />
          {images.length > 1 && (
            <>
              <button onClick={(e) => { e.stopPropagation(); prev(); }} className="absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-white/80 hover:bg-white shadow text-[#071e3d] text-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">‹</button>
              <button onClick={(e) => { e.stopPropagation(); next(); }} className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-white/80 hover:bg-white shadow text-[#071e3d] text-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">›</button>
              <div className="absolute bottom-2 right-2 bg-black/40 text-white text-[10px] px-2 py-0.5 rounded-full">{active + 1} / {images.length}</div>
            </>
          )}
          <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <div className="bg-black/40 text-white text-[9px] px-2 py-1 rounded-full flex items-center gap-1">
              <svg viewBox="0 0 24 24" fill="white" width="9" height="9"><path d="M15.5 14h-.79l-.28-.27A6.471 6.471 0 0 0 16 9.5 6.5 6.5 0 1 0 9.5 16c1.61 0 3.09-.59 4.23-1.57l.27.28v.79l5 4.99L20.49 19l-4.99-5zm-6 0C7.01 14 5 11.99 5 9.5S7.01 5 9.5 5 14 7.01 14 9.5 11.99 14 9.5 14z" /></svg>
              Büyüt
            </div>
          </div>
        </div>
        {images.length > 1 && (
          <div className="flex gap-1.5 p-2 bg-white border-t border-[#dde4ef] overflow-x-auto">
            {images.map((url, i) => (
              <button key={i} onClick={() => setActive(i)} className={`w-12 h-12 rounded border-2 overflow-hidden shrink-0 transition-all ${i === active ? "border-[#0a4c8a]" : "border-[#dde4ef] hover:border-[#9aa3af]"}`}>
...
export default function ProductDetailPage() {
  ...
  // sectorId we came from — used for breadcrumb + related products
  const fromSectorId = searchParams.get("from");

  const [product, setProduct]           = useState(null);
  const [fromSector, setFromSector]     = useState(null); // the sector we navigated from
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // 1. The one we came from (?from=sectorId) — preferred
        // 2. Fallback: first sector the product belongs to
        const sectorLinks = productData.sectors ?? [];
        let targetSectorId = fromSectorId;

        if (!targetSectorId && sectorLinks.length > 0) {
          targetSectorId = sectorLinks[0].sectorId;
        }

        // Find the sector object from the product's sector list
        const matchedLink = sectorLinks.find((s) => s.sectorId === targetSectorId);
        if (matchedLink?.sector) {
          setFromSector(matchedLink.sector);
        } else if (targetSectorId) {
          // Fetch sector info if not embedded
          try {
            const sRes  = await fetch(`${API}/api/public/sectors`);
            const sList = await sRes.json();
            const found = Array.isArray(sList) ? sList.find((s) => s.id === targetSectorId) : null;
            if (found) setFromSector(found);
          } catch {}
        }

        // Fetch related products from the same sector we came from
        if (targetSectorId) {
          const recRes  = await fetch(`${API}/api/public/sector-products?sectorId=${targetSectorId}`);
  ...
  const displayImages =
    Array.isArray(display?.images) && display.images.length ? display.images
    : display?.image ? [display.image]
    : Array.isArray(product?.images) && product.images.length ? product.images
    : product?.image ? [product.image]
    : [];

  const sectorName = fromSector?.name;
  const sectorId   = fromSector?.id;
  const backToSectorHref = sectorId
    ? `/catalogue?sector=${encodeURIComponent(sectorName ?? sectorId)}`
    : "/catalogue";
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-[#9aa3af] flex-wrap">
            ...
            {sectorName && (
              <>
                ...
                <Link href={backToSectorHref} className="hover:text-[#0a4c8a] transition-colors">
                  {sectorName}
                </Link>
              </>
            )}
      ...
      <div className="max-w-5xl mx-auto px-4 py-4 md:px-8 md:py-8">
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-[#dde4ef] p-12 text-center">
            <p className="text-[#9aa3af] text-sm">{t.not_found}</p>
            <Link href="/products" className="mt-3 inline-block text-[#1e88e5] text-sm font-semibold">← {t.products_link}</Link>
          </div>
        ) : (
          <>
                ...
                <div className="flex-1 p-4 md:p-7 flex flex-col">
                  {isGroup && (
                    <div className="inline-flex items-center gap-1.5 bg-[#eef6ff] border border-[#bdd9f5] text-[#0a4c8a] text-[10px] font-bold tracking-widest uppercase px-3 py-1 rounded-full mb-4 self-start">
                      {variants.length} {t.variants_label}
                    </div>
                  )}
                    ...
                    <div className="mt-5">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {variants.map((variant) => {
                          ...
                          const vImg = Array.isArray(variant.images) && variant.images.length ? variant.images[0] : variant.image || null;
                          return (
                            <button
                              key={variant.id}
                              onClick={() => handleVariantClick(variant)}
                              className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border text-left transition-all
                                ${isSelected ? "border-[#0a4c8a] bg-[#eef6ff]" : "border-[#dde4ef] bg-white hover:border-[#0a4c8a]/40"}`}
                            >
                              {vImg && (
                              ...
                              <div>
                                <p className={`text-[11px] font-bold uppercase tracking-wide leading-tight ${isSelected ? "text-[#0a4c8a]" : "text-[#071e3d]"}`}>{variant.name}</p>
            ...
            >
              <div className="flex items-center gap-3 md:gap-4">
                <div className="w-9 h-9 rounded-lg bg-[#1e88e5] flex items-center justify-center shrink-0">
                  <svg viewBox="0 0 24 24" fill="white" width="16" height="16"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z" /></svg>
                </div>
                <p className="text-[12px] md:text-[13px] font-medium text-white/90">{t.cta_text}</p>
              </div>
              <svg viewBox="0 0 24 24" fill="white" width="18" height="18" className="shrink-0 ml-4 md:ml-6 opacity-50 group-hover:opacity-100 group-hover:translate-x-1 transition-all">
              ...
              <div className="mt-10">
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className="text-[10px] font-bold tracking-[0.2em] uppercase text-[#1e88e5]">
                    {sectorName} {t.from_sector}
                  </span>
                </div>
                <h2 className="font-condensed text-[22px] font-extrabold uppercase text-[#071e3d] mb-5">{t.you_may_like}</h2>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {recommended.map((rec) => {
                    const recImg = Array.isArray(rec.images) && rec.images.length ? rec.images[0] : rec.image || null;
                    return (
                      <Link
                        key={rec.id}
                        href={`/products/${rec.id}${sectorId ? `?from=${sectorId}` : ""}`}
                      ...
                      >
                        <div className="h-28 md:h-36 bg-[#f8fafc] flex items-center justify-center overflow-hidden border-b border-[#dde4ef]">
                          {recImg ? (
                            <img src={recImg} alt={rec.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                          ) : (
                        ...
                        <div className="p-3 md:p-4">
                          {rec.isGroup && (
                            <span className="inline-block text-[9px] font-bold tracking-widest uppercase text-[#0a4c8a] bg-[#eef6ff] px-2 py-0.5 rounded mb-2">{t.series_label}</span>
                          )}
                          <h3 className="text-[12px] font-bold uppercase tracking-wide text-[#071e3d] leading-tight line-clamp-2 group-hover:text-[#0a4c8a] transition-colors mb-2">{rec.name}</h3>
                          {rec.description && <p className="text-[11px] text-[#9aa3af] line-clamp-2 leading-relaxed">{rec.description}</p>}
                          <div className="mt-3 pt-3 border-t border-[#f0f3f8] flex items-center justify-between">
                            {rec.price
                              ...
                              : <span className="text-[11px] text-[#9aa3af]">{rec._count?.subProducts ?? 0} {t.variants_label.toLowerCase()}</span>
                            }
