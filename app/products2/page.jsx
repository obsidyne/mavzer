...
function GroupsContent() {
  ...
  // navigation stack: each entry = { type: 'groups' | 'subProducts', id?, label?, items }
  ...
  const handleGroupClick = useCallback(async (group) => {
    // If has subProducts/children, drill in; otherwise go to product detail
    if (group.isGroup || group._count?.subProducts > 0 || group.subProducts?.length > 0) {
  ...
  const handleProductClick = useCallback((product) => {
    if (product.isGroup && (product._count?.subProducts > 0)) {
